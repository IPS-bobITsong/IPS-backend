...
class QuizConfidenceActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_quiz_confidence)
        ...
        findViewById<ImageButton>(R.id.checkAnswerBtn).setOnClickListener {
            if (findViewById<RadioButton>(R.id.option3).isChecked) {
                intent = Intent(this, CorrectActivity::class.java)
                startActivity(intent)
            }else {
                var correctanswer = findViewById<RadioButton>(R.id.option3).text
