...
class QuizSchoolActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_quiz_school)
        ...
        findViewById<ImageButton>(R.id.checkAnswerBtn).setOnClickListener {
            if (findViewById<RadioButton>(R.id.option4).isChecked) {
                intent = Intent(this, CorrectActivity::class.java)
                startActivity(intent)
            }else {
                var correctanswer = findViewById<RadioButton>(R.id.option4).text
